from random import random
from .BaseController import BaseController
from .ProjectController import ProjectController
from fastapi import UploadFile
from models import ResponseSignal
import os
import re

class DataController(BaseController):
    def __init__(self):
        super().__init__()
        self.size_scale = 1048576 # convert MB to bytes




    def validate_uploaded_file(self, file: UploadFile):
        if file.content_type not in self.app_settings.FILE_ALLOWED_TYPES:
            return False, ResponseSignal.File_Type_Not_Allowed.value


        if file.size > self.app_settings.FILE_MAX_SIZE * self.size_scale:

            return False, ResponseSignal.File_Size_Exceeds_Limit.value


        return True, ResponseSignal.File_Validation_Passed.value


    def genrate_unique_file_name(self, orig_file_name: str, project_id: str):
        random_file_name = self.generate_random_string()
        project_path = ProjectController().get_project_path(project_id=project_id)

        cleand_file_name = self.get_clean_file_name(orig_file_name=orig_file_name)
        new_file_path = os.path.join(project_path,
         random_file_name + "_" + cleand_file_name
         )

        while os.path.exists(new_file_path):
            random_file_name = self.generate_random_string()
            new_file_path = os.path.join(project_path,
            random_file_name + "_" + cleand_file_name
            )    
        return new_file_path    


    def get_clean_file_name(self, orig_file_name: str):
        # remove special characters , : * ? " < > |
        clean_file_name = re.sub(r'[\\/:*?"<>|]', '', orig_file_name)
        # remove leading and trailing spaces
        clean_file_name = clean_file_name.strip()
        # replace spaces with underscores
        clean_file_name = clean_file_name.replace(" ", "_")

        return clean_file_name











        

